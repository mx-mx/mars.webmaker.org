...
//	}
	...
	function TwoBody(inputOptions) {
		/*protected double */this.a; // sma in km
		/*protected double */this.e; // eccentricity
		/*protected double */this.i; // inclination in radians
		/*protected double */this.raan; // right ascension of ascending node in radians
		/*protected double */this.w; // argument of perigee in radians
		/*public double */this.ta; // true anomaly in radians
		/*protected double */this.mu = 398600.4415; // default: earth GM in km^3/s^2 (value from JGM-3
		/*protected double */this.steps = 500.;

		if ((inputOptions.mu != null) && (inputOptions.r != null) && (inputOptions.v != null)) {
			this.TwoBody3(inputOptions.mu, inputOptions.r, inputOptions.v);
		} else if ((inputOptions.r != null) && (inputOptions.v != null)) {
			this.TwoBody2(inputOptions.r, inputOptions.v);
		}
	}
	...
	TwoBody.prototype.TwoBody2 = function(/*VectorN*/ r, /*VectorN*/ v)
	{
		this.rv2Elements(r, v);
	};
	 ...
	 * @param v Velocity vector.
	 */
	TwoBody.prototype.TwoBody3 = function(/*double*/ mu, /*VectorN*/ r, /*VectorN*/ v)
	{
		this.mu = mu;
		this.rv2Elements(r, v);
	};
...
//	}
	 ...
	 * @param v Velocity Vector.
	 */
	/*private void */TwoBody.prototype.rv2Elements = function(/*VectorN*/ r, /*VectorN*/ v)
	{
		r.checkVectorDimensions_n(3);
		v.checkVectorDimensions_n(3);
		//Constants c = new Constants();
		var /*VectorN*/ evec = new VectorN({n:3}); // e vector
		var /*VectorN*/ k = new VectorN({n:3}); // unit vector in z direction

		this.rv = new VectorN({y:r, z:v});

		var /*double*/ rmag = r.mag();
		var /*double*/ vmag = v.mag();
		var /*double*/ energy = vmag * vmag / 2.0 - this.mu / rmag;

		k.x[0] = 0.0;
		k.x[1] = 0.0;
		k.x[2] = 1.0;

		var /*VectorN*/ h = r.crossProduct(v);
		var /*VectorN*/ n = k.crossProduct(h);

		var /*double*/ rdotv = r.dotProduct(v);

		var /*double*/ q1 = (vmag * vmag - this.mu / rmag) / this.mu;
		var /*double*/ q2 = rdotv / this.mu;
		...
		if (n.x[1] < 0.0)
		{
			this.raan = 2.0 * Constants$pi - this.raan;
		}

		this.w = Math.acos(n.dotProduct(evec) / (n.mag() * this.e));
		if (evec.x[2] < 0.0)
		{
			this.w = 2.0 * Constants$pi - this.w;
		}

		if (this.i == 0.0) // equatorial orbit, things blow up
		{
		...
		if (this.i == Constants$pi) // equatorial orbit, things blow up
		{
			//		   System.out.println("KeplerElements: equatorial orbit, RAAN no good");
			this.raan = 0.0;
			this.w = Math.acos(evec.x[0] / e);
		...
		this.ta = Math.acos(evec.dotProduct(r) / (this.e * rmag));
		if (rdotv < 0.0)
		{
...
//	 */
	...
	/*public double */TwoBody.prototype.meanMotion = function()
	{
		var /*double*/ acubed = this.a * this.a * this.a;
		var /*double*/ n = Math.sqrt(this.mu / acubed);
		return n;
	};
	...
	/*public double */TwoBody.prototype.period = function()
	{
		var /*double*/ n = this.meanMotion();
		var /*double*/ period = 2.0 * Constants$pi / n;
		return period;
	};
	...
	/*public void */TwoBody.prototype.printVector = function(/*String*/ title)
	{
	...
	/*public void */TwoBody.prototype.print = function(/*String*/ title)
	{
	...
	/*public Matrix */TwoBody.prototype.PQW2ECI = function()
	{
		var /*double*/ cw = Math.cos(this.w);
		var /*double*/ sw = Math.sin(this.w);
		var /*double*/ craan = Math.cos(this.raan);
		var /*double*/ sraan = Math.sin(this.raan);
		var /*double*/ ci = Math.cos(this.i);
		var /*double*/ si = Math.sin(this.i);

		var /*Matrix*/ out = new Matrix(3, 3);
...
//	 */
	...
	/*public double[] */TwoBody.prototype.randv = function()
	{
		var /*double*/ p = this.a * (1.0 - this.e * this.e);
		var /*double*/ cta = Math.cos(this.ta);
		var /*double*/ sta = Math.sin(this.ta);
		var /*double*/ opecta = 1.0 + this.e * cta;
		var /*double*/ sqmuop = Math.sqrt(this.mu / p);

		var /*VectorN*/ xpqw = new VectorN({n:6});
		xpqw.x[0] = p * cta / opecta;
		xpqw.x[1] = p * sta / opecta;
		xpqw.x[2] = 0.0;
		xpqw.x[3] = -sqmuop * sta;
		xpqw.x[4] = sqmuop * (this.e + cta);
		xpqw.x[5] = 0.0;

		var /*Matrix*/ cmat = this.PQW2ECI();

		var /*VectorN*/ rpqw = new VectorN({x:xpqw.x[0], y:xpqw.x[1], z:xpqw.x[2]});
		var /*VectorN*/ vpqw = new VectorN({x:xpqw.x[3], y:xpqw.x[4], z:xpqw.x[5]});

		var /*VectorN*/ rijk = cmat.times(rpqw);
		var /*VectorN*/ vijk = cmat.times(vpqw);

		var /*double[]*/ out = new Array(6);//double[6];
...
//	}
	...
	/*public static double*/function solveKepler(/*double*/ mean_anomaly, /*double*/ ecc)
	{
		...
		var /*double*/ de = 1000.0;

		var /*double*/ ea = mean_anomaly;
		var /*double*/ old_m = mean_anomaly;

		while ((it < maxit) && (Math.abs(de) > 1.0E-10))
		{
			var /*double*/ new_m = ea - ecc * Math.sin(ea);
	 ...
	 */
	/*public void */TwoBody.prototype.propagate = function(/*double*/ t0, /*double*/ tf, /*Printable*/ pr, /*boolean*/ print_switch)
	{
		var /*double[]*/ temp = new Array(6);//double[6];


		// Determine step size
		var /*double*/ n = this.meanMotion();
		var /*double*/ period = this.period();
		var /*double*/ dt = period / this.steps;
		...
		var /*double*/ sqrome2 = Math.sqrt(1.0 - this.e * this.e);
		var /*double*/ cta = Math.cos(this.ta);
		var /*double*/ sta = Math.sin(this.ta);
		var /*double*/ sine0 = (sqrome2 * sta) / (1.0 + this.e * cta);
		var /*double*/ cose0 = (this.e + cta) / (1.0 + this.e * cta);
		var /*double*/ e0 = Math.atan2(sine0, cose0);

		var /*double*/ ma = e0 - this.e * Math.sin(e0);
		...
		{
			ma = ma + n * dt;
			var /*double*/ ea = solveKepler(ma, this.e);

			var /*double*/ sinE = Math.sin(ea);
			var /*double*/ cosE = Math.cos(ea);
			var /*double*/ den = 1.0 - this.e * cosE;

			var /*double*/ sinv = (sqrome2 * sinE) / den;
			var /*double*/ cosv = (cosE - this.e) / den;
